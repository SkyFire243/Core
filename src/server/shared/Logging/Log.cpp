...
Log::Log() :
    ...
    arenaLogFile(NULL), sqlLogFile(NULL), sqlDevLogFile(NULL), wardenLogFile(NULL)
{
    Initialize();
}
...
void Log::Initialize()
{
    ...
    if (ConfigMgr::GetBoolDefault("LogFilter_TransportMoves", true))
        m_logFilter |= LOG_FILTER_TRANSPORT_MOVES;
    if (ConfigMgr::GetBoolDefault("LogFilter_CreatureMoves", true))
        m_logFilter |= LOG_FILTER_CREATURE_MOVES;
    if (ConfigMgr::GetBoolDefault("LogFilter_VisibilityChanges", true))
